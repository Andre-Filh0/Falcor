...
namespace Color {
    ...
    inline constexpr std::string_view DarkGray  = "\033[38;5;237m";
...
public:
    ...
    void log_debug(LogFormat format, Args&&... args) {
        if (m_logger) {
            std::string msg = fmt::format(fmt::runtime(format.fmt), std::forward<Args>(args)...);
...
protected:
    ...
    inline std::string format_loc(const std::source_location& loc) const {
        ...
        return fmt::format("{}{:<12}:{:>3}{}", Color::Gray, file, loc.line(), Color::Reset);
    }
...
protected:
    std::string decorate_debug()    override { return fmt::format("{}[ DEBUG  ]{}", Color::Debug,    Color::Reset); }
    std::string decorate_info()     override { return fmt::format("{}[ INFO   ]{}", Color::Info,     Color::Reset); }
    std::string decorate_warning()  override { return fmt::format("{}[ WARN   ]{}", Color::Warn,     Color::Reset); }
    std::string decorate_error()    override { return fmt::format("{}[ ERROR  ]{}", Color::Error,    Color::Reset); }
    std::string decorate_critical() override { return fmt::format("{}[ CRIT   ]{}", Color::Critical, Color::Reset); }
};
...
protected:
    std::string decorate_debug()    override { return fmt::format("{}[ SYSDBG ]{}", Color::Debug,    Color::Reset); }
    std::string decorate_info()     override { return fmt::format("{}[ SYSINFO]{}", Color::Info,     Color::Reset); }
    std::string decorate_warning()  override { return fmt::format("{}[ SYSWRN ]{}", Color::Warn,     Color::Reset); }
    std::string decorate_error()    override { return fmt::format("{}[ SYSERR ]{}", Color::Error,    Color::Reset); }
    std::string decorate_critical() override { return fmt::format("{}[ SYSCRI ]{}", Color::Critical, Color::Reset); }
};
